...
# ==================================================================
# ==================================================================
# ÇEKİRDEK 3: FOREX & KÜRESEL PİYASALAR (TAM OTONOM ÇOKLU ENSTRÜMAN RADARI - ANLIK CANLI MOTORLU)
# =================================================================================
elif calisma_modu == "Forex & Küresel Piyasalar (Çift Yönlü)":
    # Ekranın donmasını engellemek ve canlı akışı hissetmek için 10 saniyede bir otomatik tetikler
    st_autorefresh(interval=10000, key="global_forex_multi_scan_v15_final_sabit_kaleler")
    st.markdown("## 🌐 ÇİFT YÖNLÜ OTONOM FOREX KOMUTA MERKEZİ (ANLIK CANLI AKIŞ)")
    ...
    # 🔑 BİNANCE ÜZERİNDE 7/24 KESİN VERİSİ OLAN GARANTİLİ SEMBOLLER LİSTESİ
    forex_assets = {
        "BTC/USD (Bitcoin)": "BTCUSDT",
        "ETH/USD (Ethereum)": "ETHUSDT",
        "ONS ALTIN (Gold)": "PAXGUSDT",
        "SOL/USD (Solana)": "SOLUSDT",
        "BNB/USD (Binance Coin)": "BNBUSDT",
        "XRP/USD (Ripple)": "XRPUSDT",
        "LINK/USD (Chainlink)": "LINKUSDT",
        "AVAX/USD (Avalanche)": "AVAXUSDT"
    }

    # 🛡️ Kesintisiz Canlı Veri Motoru
    def get_binance_live_data(symbol, interval="1h", limit=150):
        import requests
        import pandas as pd
        urls = [
    ...
    with fx_tab2:
        ...
        <div style="position: relative; width: 100%; margin: 0; padding: 0;">
            <div class="tradingview-widget-container" style="height: 800px !important;">
              <div class="tradingview-widget-container__widget" style="width: 100%; height: 100%;"></div>
              <script type="text/javascript" src="https://s3.tradingview.com/external-embedding/embed-widget-events.js" async>
              { "colorTheme": "light", "isWidescreen": true, "width": "100%", "height": "100%", "locale": "tr", "importanceFilter": "-1,0,1", "countryFilter": "us,eu,gb,jp,ch,ca,au,tr" }
              </script>
            </div>
        </div>
        """
    ...
    with fx_tab1:
        if st.button("🚀 Sistem Bildirim Testini Tetikle"):
            try:
                import requests
                url = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage"
                payload = {"chat_id": TELEGRAM_CHAT_ID, "text": "🎯 *SİSTEM CANLI TESTİ BAŞARILI!*", "parse_mode": "Markdown"}
                response = requests.post(url, json=payload, timeout=5)
                if response.status_code == 200: st.success("🎯 Telegram bağlantısı kuruldu.")
                else: st.error("❌ Mesaj iletilemedi.")
            except Exception as e: st.error(f"🚨 Hata: {e}")

        st.info("🔄 **7/24 Arka Plan Tarayıcısı Aktif:** Kalın düz çizgiler ASIL SABİT kaleleri, kesikli ince çizgiler yürüyen anlık momentum seviyelerini gösterir.")
        ...
        for asset_adi, asset_ticker in forex_assets.items():
            try:
                ...
                # Anlık veriyi çek
                df_fx = get_binance_live_data(asset_ticker, interval="1h", limit=150)
                    
                # 🛡️ VERİ BOŞSA SİSTEMİ ÇÖKERTME, BİR SONRAKİ ENSTRÜMANA GEÇ SİPERLİĞİ
                if df_fx.empty or len(df_fx) < 25:
                    continue
                ...
                # Matematik ve İndikatör Katmanları
                ...
                # Puanlama Motoru
                long_skor = 0.0; short_skor = 0.0; nedenler = []
                if son_fiyat > b_ust: long_skor += 3.5; nedenler.append("🟩 KRİSTAL BOX: Üst band kırıldı")
                elif son_fiyat < b_alt: short_skor += 3.5; nedenler.append("🟥 KRİSTAL BOX: Alt band kırıldı")
                if son_fiyat > yuruyen_direnc: long_skor += 1.0; nedenler.append("🎯 MOMENTUM: Anlık Direnç aşıldı")
                elif son_fiyat < yuruyen_destek: short_skor += 1.0; nedenler.append("🎯 MOMENTUM: Anlık Destek patladı")
                if son_fiyat > asıl_direnc_kale: long_skor += 1.0; nedenler.append("🏰 SABİT KALE: Ana Direnç geçildi!")
                elif son_fiyat < asıl_destek_kale: short_skor += 1.0; nedenler.append("🏰 SABİT KALE: Ana Destek kırıldı!")
                if son_fiyat > ema21 and ema21 > ema50: long_skor += 2.0; nedenler.append("🟩 TREND: EMA'lar Boğa yönlü")
                elif son_fiyat < ema21 and ema21 < ema50: short_skor += 2.0; nedenler.append("🟥 TREND: EMA'lar Ayı yönlü")
                if is_bullish_pin: long_skor += 1.0; nedenler.append("🔥 PA: Boğa Pin Bar oluştu")
                if is_bearish_pin: short_skor += 1.0; nedenler.append("❄️ PA: Ayı Pin Bar oluştu")
                ...
                if anlik_algoritma_yonu != "NÖTR (İZLE)" and st.session_state[state_sinyal_key] == "NÖTR (İZLE)":
                    ...
                    mesaj_metni = f"{'🚀' if anlik_algoritma_yonu=='LONG (YUKARI)' else '💥'} *OTONOM KIRILIM*\n\n**Enstrüman:** {asset_adi}\n**Yön:** {anlik_algoritma_yonu}\n**Giriş:** `{son_fiyat:.4f}`\n**TP:** `{hedef_tp:.4f}`\n**SL:** `{risk_sl:.4f}`"
                    telegram_sinyal_gonder(mesaj_metni)
                elif anlik_algoritma_yonu == "NÖTR (İZLE)":
                    st.session_state[state_sinyal_key] = "NÖTR (İZLE)"
                    st.session_state[state_fiyat_key] = 0.0

                # 🖥️ Seçili Olan Enstrümanın Ekrana Görsel Çizimi
                if asset_adi == secilen_forex_adi:
                    strateji_yonu = st.session_state[state_sinyal_key]
                    box_bg = "#2ECC71" if strateji_yonu == "LONG (YUKARI)" else ("#E74C3C" if strateji_yonu == "SHORT (AŞAĞI)" else "#F1C40F")
                        
                    if strateji_yonu == "LONG (YUKARI)":
                        ana_skor = long_skor; sinyal_tetik_fiyati = st.session_state[state_fiyat_key]
                        durum_msg = f"🚀 GÜÇLÜ BOĞA - {sinyal_tetik_fiyati:.4f} SEVİYESİNDEN LONG SABİTLENDİ"
                        sl_noktasi = sinyal_tetik_fiyati - (atr_val * 1.5); tp_noktasi = sinyal_tetik_fiyati + (atr_val * 3.0)
                    elif strateji_yonu == "SHORT (AŞAĞI)":
                        ana_skor = short_skor; sinyal_tetik_fiyati = st.session_state[state_fiyat_key]
                        durum_msg = f"💥 GÜÇLÜ AYI - {sinyal_tetik_fiyati:.4f} SEVİYESİNDEN SHORT SABİTLENDİ"
                        sl_noktasi = sinyal_tetik_fiyati + (atr_val * 1.5); tp_noktasi = sinyal_tetik_fiyati - (atr_val * 3.0)
                    else:
                        ana_skor = max(long_skor, short_skor); sinyal_tetik_fiyati = son_fiyat
                        durum_msg = "🟡 TEST BÖLGESİ - KIRILIM BEKLENİYOR"
                        sl_noktasi = son_fiyat - (atr_val * 2.0); tp_noktasi = son_fiyat + (atr_val * 2.0)

                    st.markdown(f"""
                        <div style="background-color: {box_bg}; padding: 22px; border-radius: 12px; text-align: center; margin-bottom: 20px;">
                            <h1 style="color: #FFFFFF !important; border: none; margin: 0; font-size: 2rem;">{asset_adi} // CANLI EKİR</h1>
                            <h4 style="color: #FFFFFF !important; border: none; margin: 6px 0 0 0;">{durum_msg} (Sistem Skor: {ana_skor:.1f}/10)</h4>
                        </div>
                    """, unsafe_allow_html=True)

                    f1, f2, f3, f4, f5 = st.columns(5)
                    f1.metric("CANLI FİYAT", f"{son_fiyat:.2f}" if son_fiyat > 50 else f"{son_fiyat:.4f}")
                    f2.metric("STRATEJİ", strateji_yonu)
                    f3.metric("ATR DEĞERİ", f"{atr_val:.4f}")
                    f4.metric("HEDEF (TP)", f"{tp_noktasi:.2f}" if son_fiyat > 50 else f"{tp_noktasi:.4f}")
                    f5.metric("STOP (SL)", f"{sl_noktasi:.2f}" if son_fiyat > 50 else f"{sl_noktasi:.4f}")

                    sol_p, sag_p = st.columns([1, 2])
                    with sol_p:
                        st.markdown("### 🧠 Çift Yönlü Analiz")
                        st.write(f"**🟢 Long Gücü:** `{long_skor} / 10`")
                        st.write(f"**🔴 Short Gücü:** `{short_skor} / 10`")
                        st.markdown("#### 🔍 Tetik Gerekçeleri")
                        for neden in nedenler: st.write(f"- {neden}")
                        st.markdown("#### 🏰 Sabit Kaleler")
                        st.error(f"🏰 Üst Direnç: `{asıl_direnc_kale:.4f}`")
                        st.info(f"🏰 Alt Destek: `{asıl_destek_kale:.4f}`")

                    with sag_p:
                        st.markdown("### 📈 Canlı Grafik Haritası")
                        import plotly.graph_objects as go
                        from plotly.subplots import make_subplots
                        
                        fig_fx = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.06, row_heights=[0.7, 0.3])
                        x_str = [t.strftime('%Y-%m-%d %H:%M') for t in df_fx.index]
                        
                        fig_fx.add_trace(go.Candlestick(x=x_str, open=df_fx['Open'], high=df_fx['High'], low=df_fx['Low'], close=df_fx['Close'], name="Mumluk"), row=1, col=1)
                        fig_fx.add_trace(go.Scatter(x=x_str, y=[asıl_direnc_kale]*len(df_fx), line=dict(color='#8B0000', width=3), name="🏰 Asıl Direnç"), row=1, col=1)
                        fig_fx.add_trace(go.Scatter(x=x_str, y=[asıl_destek_kale]*len(df_fx), line=dict(color='#006400', width=3), name="🏰 Asıl Destek"), row=1, col=1)
                        fig_fx.add_trace(go.Scatter(x=x_str, y=df_fx['Direnç_S1'], line=dict(color='#C0392B', width=1.3, dash='dot'), name="Yürüyen Direnç"), row=1, col=1)
                        fig_fx.add_trace(go.Scatter(x=x_str, y=df_fx['Destek_D1'], line=dict(color='#27AE60', width=1.3, dash='dot'), name="Yürüyen Destek"), row=1, col=1)
                        fig_fx.add_trace(go.Scatter(x=x_str, y=df_fx['box_ust'], line=dict(color='#8E44AD', width=1.2, dash='dash'), name="Box Üst"), row=1, col=1)
                        fig_fx.add_trace(go.Scatter(x=x_str, y=df_fx['box_alt'], line=dict(color='#8E44AD', width=1.2, dash='dash'), name="Box Alt"), row=1, col=1)
                        fig_fx.add_trace(go.Scatter(x=x_str, y=df_fx['EMA21'], line=dict(color='#E67E22', width=1.2), name="EMA 21"), row=1, col=1)
                        fig_fx.add_trace(go.Scatter(x=x_str, y=df_fx['EMA50'], line=dict(color='#3498DB', width=1.2), name="EMA 50"), row=1, col=1)
                        
                        if strateji_yonu != "NÖTR (İZLE)":
                            c_x = [x_str[-20], x_str[-1]]
                            fig_fx.add_trace(go.Scatter(x=c_x, y=[tp_noktasi, tp_noktasi], line=dict(color='#2ECC71', width=3), name="TP"), row=1, col=1)
                            fig_fx.add_trace(go.Scatter(x=c_x, y=[sl_noktasi, sl_noktasi], line=dict(color='#E74C3C', width=3), name="SL"), row=1, col=1)
                        
                        fig_fx.add_trace(go.Scatter(x=x_str, y=df_fx['RSI'], line=dict(color='#16A085', width=1.5), name="RSI"), row=2, col=1)
                        fig_fx.update_layout(xaxis_rangeslider_visible=False, height=550, template="plotly_white", margin=dict(l=5, r=5, t=5, b=5), xaxis=dict(type="category", fixedrange=False))
                        st.plotly_chart(fig_fx, use_container_width=True)
            except Exception as loop_err:
                continue # Tek bir enstrümanda veri sıkıntısı olursa kodu patlatmaz, diğerine geçer.
