...
# =================================================================================
# =================================================================================
# ÇEKİRDEK 3: FOREX & KÜRESEL PİYASALAR MODU (TAMAMEN AKTİF GRAFİKLER)
# =================================================================================
elif calisma_modu == "Forex & Küresel Piyasalar (Çift Yönlü)":
    st_autorefresh(interval=30000, limit=200, key="forex_canli_yenileme")
    st.markdown("### 🌍 FOREX & KÜRESEL EMTAA RADARI (ÇİFT YÖNLÜ SİSTEM)")
    ...
    for asset_adi, asset_ticker in forex_assets.items():
        try:
            ...
            if not df_fx.empty and len(df_fx) > 20:
                df_fx['EMA10'] = df_fx['Close'].ewm(span=10, adjust=False).mean()
                df_fx['EMA30'] = df_fx['Close'].ewm(span=30, adjust=False).mean()
                
                delta_fx = df_fx['Close'].diff()
                gain_fx = delta_fx.clip(lower=0)
                loss_fx = -delta_fx.clip(upper=0)
                rsi_fx = 100 - (100 / (1 + (gain_fx.ewm(com=13, adjust=False).mean() / loss_fx.ewm(com=13, adjust=False).mean())))
                
                son_fiyat = float(df_fx['Close'].iloc[-1])
                son_rsi = float(rsi_fx.iloc[-1])
                ema10_val = float(df_fx['EMA10'].iloc[-1])
                ema30_val = float(df_fx['EMA30'].iloc[-1])
                
                if ema10_val > ema30_val and son_rsi > 50:
                    durum_text = "🟢 LONG (GÜÇLÜ BOĞA)"
                    renk_sinyal = "green"
                elif ema10_val < ema30_val and son_rsi < 50:
                    durum_text = "🔴 SHORT (GÜÇLÜ AYI)"
                    renk_sinyal = "red"
                else:
                    durum_text = "🟡 NÖTR (YATAY KANAL)"
                    renk_sinyal = "orange"
                    
                fx_radar_sonuclari.append({
                    "Varlık Adı": asset_adi,
                    "Ticker": asset_ticker,
                    "Son Fiyat": son_fiyat,
                    "RSI (14)": son_rsi,
                    "Yön Eğilimi": durum_text,
                    "Dataframe": df_fx
                })
        except:
            continue
            
    if fx_radar_sonuclari:
        # Önce Tüm Küresel İzleme Listesini Tablo Olarak Sunuyoruz
        izleme_verisi = []
        for item in fx_radar_sonuclari:
            izleme_verisi.append({
                "Küresel Varlık": item["Varlık Adı"],
                "Anlık Fiyat": f"{item['Son Fiyat']:.4f}",
                "RSI Durumu": f"{item['RSI (14)']:.2f}",
                "Sistem Sinyali": item["Yön Eğilimi"]
            })
        st.dataframe(pd.DataFrame(izleme_verisi), use_container_width=True)
        
        st.markdown("### 📈 SEÇİLEN KÜRESEL VARLIK GRAFİK ANALİZİ")
        secilen_fx = st.selectbox("Detaylı Grafiğini İncelemek İstediğiniz Varlığı Seçin:", [x["Varlık Adı"] for x in fx_radar_sonuclari])
        
        # Seçilen varlığın verilerini çekip jilet gibi grafiğe döküyoruz
        grafik_item = next(item for item in fx_radar_sonuclari if item["Varlık Adı"] == secilen_fx)
        df_target = grafik_item["Dataframe"]
        
        fig_fx = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.05, row_heights=[0.7, 0.3])
        fig_fx.add_trace(go.Candlestick(x=df_target.index, open=df_target['Open'], high=df_target['High'], low=df_target['Low'], close=df_target['Close'], name="Fiyat"), row=1, col=1)
        fig_fx.add_trace(go.Scatter(x=df_target.index, y=df_target['EMA10'], name="EMA 10", line=dict(color='blue', width=1.5)), row=1, col=1)
        fig_fx.add_trace(go.Scatter(x=df_target.index, y=df_target['EMA30'], name="EMA 30", line=dict(color='orange', width=1.5)), row=1, col=1)
        
        # Alt panele RSI indikatörünü jilet gibi çizdiriyoruz
        fig_fx.add_trace(go.Scatter(x=df_target.index, y=df_target['Close'].diff().clip(lower=0).ewm(com=13, adjust=False).mean() / (df_target['Close'].diff().clip(lower=0).ewm(com=13, adjust=False).mean() + (-df_target['Close'].diff().clip(upper=0)).ewm(com=13, adjust=False).mean()) * 100, name="RSI", line=dict(color='purple', width=1.5)), row=2, col=1)
        ...
        fig_fx.update_layout(height=500, template="plotly_white", xaxis_rangeslider_visible=False, margin=dict(l=10, r=10, t=10, b=10))
        st.plotly_chart(fig_fx, use_container_width=True)
    else:
        st.info("Küresel FX verileri şu an güncelleniyor, lütfen bekleyin...")


# =================================================================================
# ÇEKİRDEK 4: ULTRA FXMATIK (QUANT MATRIX MODU - TAM EKSİKSİZ VE 1:2 RASYOLU)
# =================================================================================
elif calisma_modu == "Ultra FXMatik (Quant Matrix)":
    st_autorefresh(interval=25000, limit=300, key="ultra_fx_yenileme")
    st.markdown("### ⚡ ULTRA FXMATIK QUANT MATRIX ENGINE")
    st.write("Dinamik Momentum Kapsülleri ve Volatilite Kanalları Üzerinden 1:2 Risk/Ödül Matrisiyle Çalışır.")
    
    ultra_sonuclar = []
    
    for name, ticker in forex_assets.items():
        try:
            df_fx = get_yahoo_clean_data(ticker)
                
            if not df_fx.empty and len(df_fx) > 25:
                # Dinamik Seviyeler (Momentum Takibi - Son 15 Mum)
                df_fx['Direnç_S1'] = df_fx['High'].rolling(window=15).max().shift(1)
                df_fx['Destek_D1'] = df_fx['Low'].rolling(window=15).min().shift(1)
                
                # Volatilite Kanalları (ATR Alternatifi Standart Sapma Bandı)
                df_fx['Orta_Kanal'] = df_fx['Close'].rolling(window=20).mean()
                df_fx['Std_Dev'] = df_fx['Close'].rolling(window=20).std()
                df_fx['Kanal_Ust'] = df_fx['Orta_Kanal'] + (df_fx['Std_Dev'] * 1.8)
                df_fx['Kanal_Alt'] = df_fx['Orta_Kanal'] - (df_fx['Std_Dev'] * 1.8)
                
                # RSI Tabanlı Aşırı Uç Analizi
                delta_u = df_fx['Close'].diff()
                gain_u = delta_u.clip(lower=0)
                loss_u = -delta_u.clip(upper=0)
                df_fx['RSI_FX'] = 100 - (100 / (1 + (gain_u.ewm(com=13, adjust=False).mean() / loss_u.ewm(com=13, adjust=False).mean())))
                
                fiyat_anlik = float(df_fx['Close'].iloc[-1])
                direnc_s1 = float(df_fx['Direnç_S1'].iloc[-1])
                destek_d1 = float(df_fx['Destek_D1'].iloc[-1])
                k_ust = float(df_fx['Kanal_Ust'].iloc[-1])
                k_alt = float(df_fx['Kanal_Alt'].iloc[-1])
                rsi_fx = float(df_fx['RSI_FX'].iloc[-1])
                
                durum_text = "YATAY KANAL (BEKLE)"
                tp_hedef = 0.0
                sl_stop = 0.0
                hiz_skoru = 0
                
                # STRATEJİK RADİKAL YÖN TAYİNİ
                if fiyat_anlik >= direnc_s1 and rsi_fx > 52:
                    durum_text = "🚀 GÜÇLÜ BOĞA KIRILIMI (LONG)"
                    # 1:2 RASYOLU MATEMATİKSEL KURULUM (Mevcut Risk Genişliğinin 2 Katı Kâr Hedefi)
                    risk_birimi = fiyat_anlik - destek_d1 if (fiyat_anlik - destek_d1) > 0 else fiyat_anlik * 0.01
                    sl_stop = destek_d1
                    tp_hedef = fiyat_anlik + (risk_birimi * 2.0)
                    hiz_skoru = int(min(100, max(10, rsi_fx + 15)))
                    
                elif fiyat_anlik <= destek_d1 and rsi_fx < 48:
                    durum_text = "🔴 GÜÇLÜ AYI KIRILIMI (SHORT)"
                    risk_birimi = direnc_s1 - fiyat_anlik if (direnc_s1 - fiyat_anlik) > 0 else fiyat_anlik * 0.01
                    sl_stop = direnc_s1
                    tp_hedef = fiyat_anlik - (risk_birimi * 2.0)
                    hiz_skoru = int(min(100, max(10, (100 - rsi_fx) + 15)))
                    
                elif fiyat_anlik >= k_ust and rsi_fx > 68:
                    durum_text = "⚠️ YALANCI KIRILIM (BOĞA TUZAĞI)"
                    tp_hedef = df_fx['Orta_Kanal'].iloc[-1]
                    sl_stop = k_ust * 1.005
                    hiz_skoru = 40
                    
                ultra_sonuclar.append({
                    "Asset": name,
                    "Fiyat": fiyat_anlik,
                    "Sinyal": durum_text,
                    "TP (Kâr Al)": tp_hedef,
                    "SL (Zarar Kes)": sl_stop,
                    "İvme": hiz_skoru,
                    "DF": df_fx
                })
        except:
    ...
    if ultra_sonuclar:
        tablo_matrisi = []
        for u in ultra_sonuclar:
            tablo_matrisi.append({
                "Varlık Matrisi": u["Asset"],
                "Quant Fiyat": f"{u['Fiyat']:.4f}",
                "Stratejik Sinyal": u["Sinyal"],
                "Hedef (TP)": f"{u['TP (Kâr Al)']:.4f}" if u['TP (Kâr Al)'] > 0 else "N/A",
                "Durdurma (SL)": f"{u['SL (Zarar Kes)']:.4f}" if u['SL (Zarar Kes)'] > 0 else "N/A",
                "Güç Skoru": f"%{u['İvme']}" if u['İvme'] > 0 else "N/A"
            })
        st.dataframe(pd.DataFrame(tablo_matrisi), use_container_width=True)
        
        st.markdown("### ⚡ QUANT MATRIX DİNAMİK GRAFİKÖRÜ")
        secilen_ultra = st.selectbox("Quant Grafiği Çizdirilecek Varlık:", [x["Asset"] for x in ultra_sonuclar])
        
        ultra_item = next(item for item in ultra_sonuclar if item["Asset"] == secilen_ultra)
        df_ultra_plot = ultra_item["DF"]
        durum_text = ultra_item["Sinyal"]
        tp_hedef = ultra_item["TP (Kâr Al)"]
        sl_stop = ultra_item["SL (Zarar Kes)"]
        hiz_skoru = ultra_item["İvme"]
        rsi_fx = float(df_ultra_plot['RSI_FX'].iloc[-1])
        
        fig_ultra = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.06, row_heights=[0.75, 0.25])
        fig_ultra.add_trace(go.Candlestick(x=df_ultra_plot.index, open=df_ultra_plot['Open'], high=df_ultra_plot['High'], low=df_ultra_plot['Low'], close=df_ultra_plot['Close'], name="Kandil"), row=1, col=1)
        fig_ultra.add_trace(go.Scatter(x=df_ultra_plot.index, y=df_ultra_plot['Direnç_S1'], name="Dinamik Direnç (S1)", line=dict(color='red', width=1.5, dash='dash')), row=1, col=1)
        fig_ultra.add_trace(go.Scatter(x=df_ultra_plot.index, y=df_ultra_plot['Destek_D1'], name="Dinamik Destek (D1)", line=dict(color='green', width=1.5, dash='dash')), row=1, col=1)
        fig_ultra.add_trace(go.Scatter(x=df_ultra_plot.index, y=df_ultra_plot['Kanal_Ust'], name="Kanal Üst Sınır", line=dict(color='rgba(150,150,150,0.5)', width=1)), row=1, col=1)
        fig_ultra.add_trace(go.Scatter(x=df_ultra_plot.index, y=df_ultra_plot['Kanal_Alt'], name="Kanal Alt Sınır", line=dict(color='rgba(150,150,150,0.5)', width=1)), row=1, col=1)
        
        fig_ultra.add_trace(go.Scatter(x=df_ultra_plot.index, y=df_ultra_plot['RSI_FX'], name="Quant RSI", line=dict(color='teal', width=1.5)), row=2, col=1)
        fig_ultra.add_hline(y=50, line_dash="dot", line_color="gray", row=2, col=1)
        
        fig_ultra.update_layout(height=550, template="plotly_white", xaxis_rangeslider_visible=False, margin=dict(l=10, r=10, t=10, b=10))
        ...
        if "GÜÇLÜ BOĞA" in durum_text or "GÜÇLÜ AYI" in durum_text:
            ...
            s3.write(f"RSI Seviyesi: {rsi_fx:.2f}")
        else:
            s1.info("⌛ Formasyon Aranıyor (Nötr Bant)")
            s2.write(f"Üst Dinamik Sınır: {ultra_item['Fiyat']:.4f} / {df_ultra_plot['Direnç_S1'].iloc[-1]:.4f}")
            s3.write(f"Anlık RSI Momentum: {rsi_fx:.1f}")
    else:
        st.info("Quant algoritmaları piyasa verilerini süzüyor, lütfen 2-3 saniye bekleyin...")
